...
sim: $(SOURCES)
	$(IVERILOG) -B "$${HOME}/.apio/packages/tools-oss-cad-suite/lib/ivl" -o $(TBNAME).out -D VCD_OUTPUT=$(TBNAME) -D NO_ICE40_DEFAULT_ASSIGNMENTS "$${HOME}/.apio/packages/tools-oss-cad-suite/share/yosys/ice40/cells_sim.v" $(SOURCES_TB)'
	$(VVP) $(TBNAME).out'
	
verify:
	$(IVERILOG) -B "$${HOME}/.apio/packages/tools-oss-cad-suite/lib/ivl" -o hardware.out -D VCD_OUTPUT= -D NO_ICE40_DEFAULT_ASSIGNMENTS "$${HOME}/.apio/packages/tools-oss-cad-suite/share/yosys/ice40/cells_sim.v" $(SOURCES)'
